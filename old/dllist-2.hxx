...
//
//   1) (DONE) final
//   2)  (DONE) inherit publicly (but not virtually) from dllist_node_ptr_only<T>
//   3) (DONE) declare a private instance of type T called datum_
//   4) (DONE) declare a public type alias called "xorptr_type" that is of
...
class dllist_node final: public dllist_node_ptr_only<T>
{
    private:
        T datum_;
public:
     	using xorptr_type = dllist_node_ptr_only<T>;
  ...
  //   2) Initialize datum_ with datum using copy construction.
    explicit dllist_node(T const& datum):dllist_node_ptr_only(nullptr),datum_(std::copy(datum))
  ...
  //   2) Initialize datum_ with datum using move construction.
    explicit dllist_node(T&& datum):dllist_node_ptr_only(nullptr,nullptr),datum_(std::move(datum))

  // No code can appear inside the next constructor's function body (braces).
  // Do initialize the class as follows before the opening brace:
  //   1) "call" the parent type's constructor passing prev and next values.
  //   2) Initialize datum_ with datum using copy construction.
  dllist_node(T const& datum, dllist_node* prev, dllist_node* next):dllist_node_ptr_only(prev,next),datum_(std::copy(datum))

  // No code can appear inside the next constructor's function body (braces).
  // Do initialize the class as follows before the opening brace:
  //   1) "call" the parent type's constructor passing prev and next values.
  //   2) Initialize datum_ with datum using move construction.
  dllist_node(T&& datum, dllist_node* prev, dllist_node* next):dllist_node_ptr_only(prev,next),datum_(std::move(datum))
  ...
  //   2) Initialize datum_ with datum using copy construction.
  dllist_node(T const& datum, xorptr_type const& xp):dllist_node_ptr_only(xp),datum_(std::copy(datum))

  // No code can appear inside the next constructor's function body (braces).
  // Do initialize the class as follows before the opening brace:
  //   1) "call" the parent type's constructor passing xp values.
  //   2) Initialize datum_ with datum using move construction.
  dllist_node(T&& datum, xorptr_type const& xp):dllist_node_ptr_only(xp),datum_(std::move(datum))
  ...
  //   2) Initialize datum_ with datum using move construction.
  dllist_node(T&& datum, xorptr_type&& xp):dllist_node_ptr_only(std::move(xp)),datum_(std::move(datum))
  ...
  dllist_node(dllist_node&& n):dllist_node_ptr_only(std::move(n)),datum_(std::move(datum))
  ...
  dllist_node& operator =(dllist_node&& n):dllist_node_ptr_only::operator =(n),datum_(std::move(datum))
    {
        return *this;
    }
  ...
  //
            // choose eithrt this datum_(b->datum_) or datum_(b.datum_)

  void swap(dllist_node& b):dllist_node_ptr_only::swap(std::swap(b)),datum_(b.datum_)
...
public:
  ...
  //   1) (DONE) Explictly default initialize size_ (which sets its value to zero).
  //   2) (DONE)Initialize front_ by passing in &back_ twice.
  //   3) (DONE)Initialize back_ by passing in &front_ twice.
  //
  ...
  //
  dllist():size_(0),front_(&back_,&back_),back_(&front_,&front_)
  ...
  //
  dllist(size_type n, T const& value = T{}):dllist{}{
  ...
  {
      std::copy(il,std::back_inserter(*this));
  }
  ...
  {
      dllist<T> tmp=new dllist(std::copy(l));
      this->swap(tmp);
      return this;
  }
  ...
  {
       dllist<T> tmp=new dllist(std::move(l));
      this->swap(tmp);
      return this;
  }
  ...
  {
      dllist<T> tmp=new dllist(l,n,value)
      this->swap(tmp);
  }
  ...
  {
      dllist<T> tmp=new dllist(first,last)
      this->swap(tmp);
  }
  ...
  bool empty() const
  {
      if (this->front_==nullptr) return true;
      return false;
  }
  ...
  iterator emplace(iterator pos, Args&&... args)
  {
      ...
      // 5 Dont know how to build iterator and return it.
  ...
  void emplace_back(Args&&... args)
 {
  ...
  //      a) pos = insert(pos, *first); // error by prof () *first can not be used here
...
public:
  ...
  {
      dllist_iter<T> tmp = new dllist_iter(*this);
      tmp++;
      return tmp;
  }
  // Implement the prefix operator -- as follows:
