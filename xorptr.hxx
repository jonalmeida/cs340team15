...
#if defined(CXX_XOR_PROJECT_USE_UINTPTR_T)
template <typename T>
struct xorptr_traits final
{
    ...
    xorptr_type const& xp, pointer_type p) noexcept
  {
    return
      static_cast<T*>(
        reinterpret_cast<void*>(xp ^ reinterpret_cast<std::uintptr_t>(p))
      )
    ;
  }
    ...
    xorptr_type const& xp, const_pointer_type p) noexcept
  {
    return
      static_cast<T const*>(
        reinterpret_cast<void const*>(xp ^ reinterpret_cast<std::uintptr_t>(p))
      )
    ;
  }
};
...
struct xorptr_traits final
{
  ...
  static xorptr_type create() noexcept
  {
    return create(static_cast<pointer_type>(nullptr), static_cast<pointer_type>(nullptr));
  }
  // Create an XOR-encoded pointer value of two non-const pointer values...
  static xorptr_type create(
   pointer_type p1, pointer_type p2) noexcept
  {
    using ucharptr = unsigned char*;
   ...
   const_pointer_type p1, const_pointer_type p2) noexcept
  {
    ...
    return static_cast<xorptr_type>(const_cast<void*>(vp1));
  }
               ...
               pointer_type p) noexcept
  {
    ...
    for (int i=0; i != sizeof(ucharptr); ++i)
      ucp1[i] ^= ucp2[i];
...
class xorptr final
{
  ...
  private:
    ...
    xorptr_type xorptr_;
  public:
    xorptr() noexcept : xorptr_(traits_type::create()) {}
    explicit xorptr(pointer_type ptr1, pointer_type ptr2) noexcept : xorptr_(traits_type::create(ptr1, ptr2)) {}
		explicit xorptr(const_pointer_type ptr1, const_pointer_type ptr2) noexcept : xorptr_(traits_type::create(ptr1, ptr2)) {}
